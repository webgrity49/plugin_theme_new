...
<ul class="stssm-social-icons stssm-content-social-icons">
	...
	public static function enqueue_scripts() {
		...
		if ( $enqueue ) {
			wp_enqueue_style( 'font-awesome-free', STSSM_PLUGIN_URL . 'assets/css/all.min.css', array(), '5.15.3', 'all' );
			...
			if ( $content || $sticky ) {
				...
				if ( 30 !== $content_icons_design['w'] ) {
					$content_css .= ( '.stssm-content-social-icons .fab, .stssm-content-social-icons .fas { width: ' . absint( $content_icons_design['w'] ) . 'px; }' );
				}
				if ( 30 !== $content_icons_design['h'] ) {
					$content_css .= ( '.stssm-content-social-icons .fab, .stssm-content-social-icons .fas { height: ' . absint( $content_icons_design['h'] ) . 'px; line-height: ' . absint( $content_icons_design['h'] ) . 'px; font-size: ' . absint( $content_icons_design['h'] / 2 ) . 'px; }' );
				}
				...
				if ( $sticky ) {
					...
					if ( $apply_sticky_width ) {
						$sticky_css .= ( '.stssm-sticky-social-icons .fab, .stssm-sticky-social-icons .fas { width: ' . absint( $sticky_icons_design['w'] ) . 'px; }' );
					}
					if ( 30 !== $sticky_icons_design['h'] ) {
						$sticky_css .= ( '.stssm-sticky-social-icons .fab, .stssm-sticky-social-icons .fas { height: ' . absint( $sticky_icons_design['h'] ) . 'px; line-height: ' . absint( $sticky_icons_design['h'] ) . 'px; font-size: ' . absint( $sticky_icons_design['h'] / 2 ) . 'px; }' );
					}
					if ( 3 !== $sticky_icons_design['mt'] ) {
						$sticky_css .= ( '.stssm-sticky-social-icons li { margin-top: ' . absint( $sticky_icons_design['mt'] ) . 'px !important; }' );
					}
					if ( 3 !== $sticky_icons_design['mb'] ) {
						$sticky_css .= ( '.stssm-sticky-social-icons li { margin-bottom: ' . absint( $sticky_icons_design['mb'] ) . 'px !important; }' );
					}
					...
					if ( 'right' === $sticky_placement['place'] ) {
						...
						if ( $apply_sticky_width ) {
							$sticky_right_css = ( '.stssm-hide { -webkit-transform: translateX(' . absint( $sticky_icons_design['w'] ) . 'px); -moz-transform: translateX(' . absint( $sticky_icons_design['w'] ) . 'px); -ms-transform: translateX(' . absint( $sticky_icons_design['w'] ) . 'px); -o-transform: translateX(' . absint( $sticky_icons_design['w'] ) . 'px); transform: translateX(' . absint( $sticky_icons_design['w'] ) . 'px); }' );
						...
						if ( $apply_sticky_width ) {
							$sticky_left_css = ( '.stssm-hide { -webkit-transform: translateX(-' . absint( $sticky_icons_design['w'] ) . 'px); -moz-transform: translateX(-' . absint( $sticky_icons_design['w'] ) . 'px); -ms-transform: translateX(-' . absint( $sticky_icons_design['w'] ) . 'px); -o-transform: translateX(-' . absint( $sticky_icons_design['w'] ) . 'px); transform: translateX(-' . absint( $sticky_icons_design['w'] ) . 'px); }' );
